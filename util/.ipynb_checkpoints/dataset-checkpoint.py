...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None):    
    ...
    # print('len(image_label_list)', len(image_label_list))
...
class MultiViewSemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, train_transform=None, transform_1=None, transform_2=None, transform_3=None, transform_4=None, mode='train',use_coco=False, use_split_coco=False):
        assert mode in ['train', 'val', 'test']
    ...
    def get_view_s_x_y(self, view_support_image_list, view_support_label_list):
        s_xs_view_ = view_support_image_list
        s_ys_view_ = view_support_label_list
        s_x_view_ = s_xs_view_[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_x_view_ = torch.cat([s_xs_view_[i].unsqueeze(0), s_x_view_], 0)
        s_y_view_ = s_ys_view_[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_y_view_ = torch.cat([s_ys_view_[i].unsqueeze(0), s_y_view_], 0)
        return s_x_view_, s_y_view_
    ...
    def __getitem__(self, index):
        ...
        for k in range(self.shot):
            support_idx = random.randint(1, num_file) - 1
            support_idx_2 = random.randint(1, num_file) - 1
            support_idx = (support_idx + support_idx_2) // 2
            support_image_path = image_path
            support_label_path = label_path
            while ((support_image_path == image_path and support_label_path == label_path) or support_idx in support_idx_list):
                support_idx = random.randint(1, num_file) - 1
        ...
        raw_label = label.copy()
        view_1_support_image_list, view_1_support_label_list = list(), list()
        view_2_support_image_list, view_2_support_label_list = list(), list()
        view_3_support_image_list, view_3_support_label_list = list(), list()
        view_4_support_image_list, view_4_support_label_list = list(), list()
        if self.transform is not None:
            image, label, padding_mask = self.transform(image, label, padding_mask)
            for k in range(self.shot):
                s_i_v, s_l_v, _ = self.transform_1(support_image_list[k], support_label_list[k], support_padding_list[k])
                view_1_support_image_list.append(s_i_v)
                view_1_support_label_list.append(s_l_v)
                s_i_v, s_l_v, _ = self.transform_2(support_image_list[k], support_label_list[k], support_padding_list[k])
                view_2_support_image_list.append(s_i_v)
                view_2_support_label_list.append(s_l_v)
                s_i_v, s_l_v, _ = self.transform_3(support_image_list[k], support_label_list[k], support_padding_list[k])
                view_3_support_image_list.append(s_i_v)
                view_3_support_label_list.append(s_l_v)
                s_i_v, s_l_v, _ = self.transform_4(support_image_list[k], support_label_list[k], support_padding_list[k])
                view_4_support_image_list.append(s_i_v)
                view_4_support_label_list.append(s_l_v)
        ...
        s_x_view_1, s_y_view_1 = self.get_view_s_x_y(view_1_support_image_list, view_1_support_label_list)
        s_x_view_2, s_y_view_2 = self.get_view_s_x_y(view_2_support_image_list, view_2_support_label_list)
        s_x_view_3, s_y_view_3 = self.get_view_s_x_y(view_3_support_image_list, view_3_support_label_list)
        s_x_view_4, s_y_view_4 = self.get_view_s_x_y(view_4_support_image_list, view_4_support_label_list)
        

        if self.mode == 'train':
            return image, label, s_x, s_y, padding_mask, s_ey, subcls_list, s_x_view_1, s_y_view_1, s_x_view_2, s_y_view_2, s_x_view_3, s_y_view_3, s_x_view_4, s_y_view_4
        else:
            return image, label, s_x, s_y, padding_mask, s_ey, subcls_list, raw_label, s_x_view_1, s_y_view_1, s_x_view_2, s_y_view_2, s_x_view_3, s_y_view_3, s_x_view_4, s_y_view_4